...
#ifndef POPULATION_MANAGER_TUBESETTLER_H
#define POPULATION_MANAGER_TUBESETTLER_H
...
class PopulationManager_TubeSettler {
  protected:
    using ptr_t = ORG*;
  ...
  public:
    ...
    friend class PopulationIterator<PopulationManager_TubeSettler<ORG> >;
    using iterator = PopulationIterator<PopulationManager_TubeSettler<ORG> >;
    // Necessary overloads
    ...
    void ConfigPop(double width, double height, int max_pop_size = 10, double max_org_radius = 10, double detach_on_birth = true, double point_mutation_rate = 0.001, double reproduction_cost = 1.0, int max_resource_count = 0, int max_resource_age = 1.0, double resource_energy_content = 1.0, bool use_gravity = true) {
      /*
    ...
    void Update() {
      ...
      for (auto *org : pop) {
        double terminal_velocity = 5.0;
        // Add (something analogous to) gravity
        if (use_gravity && (org->GetVelocity().GetY() < terminal_velocity)) org->IncSpeed(Angle(0.0).GetPoint(1.0));
